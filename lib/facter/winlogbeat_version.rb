...
Facter.add('winlogbeat_version') do
  ...
  if File.exist?('c:\Program Files\Winlogbeat\winlogbeat.exe')
    ...
    winlogbeat_version = Facter::Core::Execution.execute('"c:\Program Files\Winlogbeat\winlogbeat.exe" version')
    winlogbeat_version = Facter::Core::Execution.execute('"c:\Program Files\Winlogbeat\winlogbeat.exe" --version') if winlogbeat_version.empty?
  end
  setcode do
